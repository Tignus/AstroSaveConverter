...
def ask_user_to_choose_in_a_list(text: str, file_list: List[str]) -> str:
    ...
    while choice_index == 0:
        ...
        try:
            ...
            verify_choice_input(
                choice_index, min_container_number, max_container_number
            )
        except ValueError:
            choice_index = 0
            Logger.logPrint(
        ...
        try:
            ...
            if work_choice == "1":
                if conversion_type == AstroConvType.WIN2STEAM:
                    try:
                        astroneer_save_folder = (
                            AstroMicrosoftSaveFolder.get_microsoft_save_folder()
                        )
                        Logger.logPrint(
                            f"Microsoft folder path: {astroneer_save_folder}", "debug"
                        )
                        while True:
                            save_path = ask_copy_target(
                                "MicrosoftAstroneerSavesBackup", "Microsoft"
                            )
                            try:
                                ...
                                Logger.logPrint(
                                    "Invalid path; please choose another backup location"
                                )
...
def ask_copy_target(folder_main_name: str, save_type: str) -> str:
    ...
    """
    Logger.logPrint(
        f"Where would you like to backup your {save_type.capitalize()} save folder ?"
    )
...
def export_save_to_xbox(save: AstroSave, from_file: str, to_path: str) -> str:
    ...
    with open(container_full_path, "r+b") as container:
        ...
        current_container_chunk_count = int.from_bytes(
...
def backup_win_before_steam_export() -> str:
    ...
    try:
        ...
        Logger.logPrint(
            "No Microsoft save folders were found. Choose where to place the converted save files so you can manually move them later."
        )
        ...
        if choice == "3":
            Logger.logPrint(
                "Launch the Microsoft version of Astroneer once, then relaunch AstroSaveConverter."
            )
            return ""
        if choice == "1":
        ...
        output_path = utils.join_paths(
            base_path, utils.create_folder_name("MicrosoftAstroneerSave")
        )
        utils.make_dir_if_doesnt_exists(output_path)
        return output_path
    Logger.logPrint(
        f"{len(folders)} different Microsoft save folders have been detected. They will all be backed up."
    )
