...
class AstroSaveContainer:
    ...
    def __init__(self, container_file_path: str) -> None:
        ...
        with open(self.full_path, "rb") as container:
            ...
            self.chunk_count = int.from_bytes(container.read(4), byteorder="little")

            # Parsing saves chunks
            current_save_name = None
            for _ in range(self.chunk_count):
                current_chunk = container.read(CHUNK_METADATA_SIZE)
                current_chunk_name = self.extract_name_from_chunk(current_chunk)
            ...
            self.save_list.append(AstroSave(current_save_name, current_chunks_names))
    ...
    def extract_name_from_chunk(self, chunk: bytes) -> str:
        ...
        utf_16_encoded_text = chunk[0 : CHUNK_METADATA_SIZE - 32].decode(
