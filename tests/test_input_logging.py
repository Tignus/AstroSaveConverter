...
def test_ask_conversion_type_logs_choice():
    with patch.object(builtins, "input", side_effect=["3", "1"]), patch(
...
def test_ask_copy_target_logs_choices():
    ...
    with patch.object(builtins, "input", side_effect=["2", "a", "/tmp"]), patch(
        "AstroSaveScenario.os.makedirs", side_effect=OSError
    ), patch(
        "AstroSaveScenario.os.path.isdir",
        side_effect=lambda p: False if p == invalid_path else real_isdir(p),
    ), patch(
        "cogs.AstroLogging.logPrint"
    ) as log_mock:
...
def test_rename_save_logs():
    save = AstroSave("OLD$2024.01.01-00.00.00", [])
    with patch.object(builtins, "input", side_effect=["newname"]), patch(
        "cogs.AstroLogging.logPrint"
    ) as log_mock:
...
def test_seek_microsoft_save_folder_logs_choice():
    with patch.object(builtins, "input", side_effect=["3", "1"]), patch(
        ...
        "cogs.AstroMicrosoftSaveFolder.get_save_folders_from_path",
        return_value=["a", "b"],
    ):
...
def test_ask_microsoft_target_folder_logs_choice():
    with patch.object(builtins, "input", side_effect=["3", "1"]), patch.dict(
        "os.environ", {"LOCALAPPDATA": "/tmp"}
    ), patch("glob.iglob", return_value=["dummy"]), patch(
        "cogs.AstroMicrosoftSaveFolder.get_save_folders_from_path",
        return_value=["a", "b"],
    ), patch(
        "cogs.AstroMicrosoftSaveFolder.get_save_details", return_value=[]
    ), patch(
        "cogs.AstroLogging.logPrint"
    ) as log_mock:
...
def test_backup_win_before_steam_export_logs_choice():
    with patch.object(builtins, "input", side_effect=["4", "3"]), patch(
        "cogs.AstroMicrosoftSaveFolder.find_microsoft_save_folders",
